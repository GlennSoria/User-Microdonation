...
export default function Dashboard() {
  ...
  // Parse user from params, provide defaults
  const user: User = params.user
    ? JSON.parse(params.user)
    : { id: 0, name: 'Guest', wallet: 0 };
  ...
  const donate = async (projectId: number, amount: number) => {
    ...
    try {
      const res = await fetch(`${API}/donate.php`, {
        ...
        body: JSON.stringify({ user_id: user.id, project_id: projectId, amount }),
      });
      const data = await res.json();
      ...
      if (data.success) {
        fetchWallet();
        fetchProjects();
        Alert.alert('Success', `Donated ₱${amount}`);
        setCustomAmounts((prev) => ({ ...prev, [projectId]: '' }));
      } else {
    ...
    return (
      ...
      >
        ...
        <View style={styles.quickDonateContainer}>
          {[1, 5, 10, 20].map((amt) => (
            <TouchableOpacity key={amt} style={styles.quickDonateButton} onPress={() => donate(item.id, amt)}>
        ...
        <View style={styles.customAmountContainer}>
          <TextInput
            ...
            onChangeText={(val) => setCustomAmounts((p) => ({ ...p, [item.id]: val }))}
            style={styles.customAmountInput}
          />
          <TouchableOpacity
            style={styles.donateButton}
            onPress={() => donate(item.id, Number(customAmounts[item.id] || 0))}
          >
  ...
  const renderHeader = () => (
    <Animated.View style={[styles.headerContent, { opacity: headerOpacity }]}>
      ...
      <View style={styles.navigationLinks}>
        ...
        >
          <Ionicons name="receipt-outline" size={24} color="#FF4500" />
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <FlatList
        ...
        contentContainerStyle={{ paddingTop: HEADER_HEIGHT_MAX + 10, paddingHorizontal: 20, paddingBottom: 20 }}
